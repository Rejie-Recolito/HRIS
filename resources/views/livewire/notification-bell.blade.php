@if(Auth::user() && Auth::user()->is_admin)
    ...
    <div class="relative ms-4">
        <button @click="open = !open; if(open){ $wire.markNotificationsRead() }" class="relative z-10 flex items-center justify-center h-8 w-8 rounded-full bg-white dark:bg-[#282828] focus:outline-none">
            ...
            @if($unreadCount > 0)
                <span x-show="!open" class="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-xs font-bold bg-red-600 text-white animate-pulse min-w-[20px] min-h-[20px]">{{ $unreadCount }}</span>
            @endif
        </button>
...
@else
    ...
    <div class="relative ms-4">
        <button @click="openUserNotifications = !openUserNotifications; if(openUserNotifications){ $wire.markNotificationsRead() }" class="relative z-10 flex items-center justify-center h-8 w-8 rounded-full bg-white dark:bg-[#282828] focus:outline-none">
            <svg class="h-5 w-5 text-gray-600 dark:text-gray-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
            @if($unreadCount > 0)
                <span x-show="!openUserNotifications" class="absolute -top-1 -right-1 inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-xs font-bold bg-red-600 text-white animate-pulse min-w-[20px] min-h-[20px]">{{ $unreadCount }}</span>
            @endif
        </button>
